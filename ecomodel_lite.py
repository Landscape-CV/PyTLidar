...
logger = logging.getLogger("Ecomodel_Lite")
logger.setLevel(logging.INFO)
f_handler = logging.FileHandler('ecomodel_lite_log.log')
...
class EcomodelLite:
    ...
    def remove_duplicates(self, point_cloud):
        """
        Filters points based on intensity.
        ...
        Args:
            ...
            intensity (float): Removes points below this intensity threshold.
        """
        outputCloud,mask = np.unique(point_cloud[:,0:3],return_index = True, axis=0)
    ...
    def save_point_cloud_las(self,point_data, name):
        """
        Saves the tile as a LAS file in the results folder.
        """

        header = laspy.LasHeader(point_format=0, version="1.4")
        header.offsets = np.min(point_data[:, :3], axis=0)
        ...
        if point_data.shape[1] >= 4:
            las.intensity = point_data[:, 3].astype(np.uint16)

        path = f"{self.results_folder}/{name}.las"
        las.write(path)
        logger.info(f"Saved tile to {path}")
    ...
    def process_tile(self, tile_path, save_data=False, show_plots=False):
        ...
        timer1 = time.time()
        _, full_data = load_point_cloud(str(path),intensity_threshold=self.intensity_threshold, full_data=True)

        timer2 = time.time()
        logger.info(f"Loading point cloud took {round(timer2 - timer1, 3)} seconds")

        full_data=model.remove_duplicates(full_data)
        timer3 = time.time()
        logger.info(f"Removing duplicates took {round(timer3 - timer2, 3)} seconds")

        full_data = model.normalize_point_cloud(full_data)
        timer4 = time.time()
        logger.info(f"Normalizing point cloud took {round(timer4 - timer3, 3)} seconds")

        full_data = model.remove_ground(full_data)
        timer5 = time.time()
        logger.info(f"Removing ground took {round(timer5 - timer4, 3)} seconds")

        toSave=full_data.copy()
        toSave= self.unnormalize_point_cloud(toSave)
        model.save_point_cloud_las(toSave,f"{path.stem}/{path.stem}_post_ground_removal")
        ...
        if full_data is None:
            ...
            return
        timer7 = time.time()
        logger.info(f"Removing leaves took {round(timer7 - timer6, 3)} seconds")
        toSave = full_data.copy()
        toSave = self.unnormalize_point_cloud(toSave)
        model.save_point_cloud_las(toSave, f"{path.stem}/{path.stem}_post_leaves_removal")
        ...
        if full_data is None or instance_labels is None:
            ...
            return
        timer9 = time.time()
        logger.info(f"Segmenting trees took {round(timer9 - timer8, 3)} seconds")
        ...
        timer10 = time.time()
        logger.info(f"Running treeqsm took {round(timer10 - timer9, 3)} seconds")
